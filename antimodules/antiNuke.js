...
const antiNuke = (client) => {
    ...
    client.on('messageCreate', async (message) => {
        ...
        const guildConfig = await antisetupCollection.findOne({ serverId: message.guild.id });

        if (guildConfig && guildConfig.badWords && guildConfig.badWords.enabled) {
            if (filter.isProfane(message.content)) {
                ...
                const embed = new EmbedBuilder()
                    ...
                    .setDescription(`User ${message.author.tag} (${message.author.id}) used a prohibited word.`)
                    .addFields(
