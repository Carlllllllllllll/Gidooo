...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .setDescription('Set the owner ID for a specific server')
    ...
    async execute(interaction) {
        if (interaction.isCommand && interaction.isCommand()) {
            ...
            if (!serverId || !ownerId) {
                return interaction.reply({ content: 'Invalid input. Please provide valid server ID and owner ID.', ephemeral: true });
            }

            try {
                await serverConfigCollection.updateOne(
                    { serverId },
                    { $set: { serverId, ownerId } },
                    { upsert: true }
                );

                interaction.reply({ content: `Server owner ID updated successfully for server ID ${serverId}.`, ephemeral: true });
            } catch (error) {
